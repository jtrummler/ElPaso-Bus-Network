...
```{r context_map, warning=FALSE, message=FALSE, results=FALSE}
...
## Current Local Bus Network

In our model, we are primarily focusing on local bus routes. Out of the 56 existing routes, 31 of them are deemed local routes.
...
acs_ep.popDense <- acs_ep %>%
  dplyr::select(c("totalPop", "geometry")) %>%
  st_transform(st_crs(el_paso))
acs_ep.popDense <- st_intersection(acs_ep.popDense, el_paso)

ggplot()+
  geom_sf(data=acs_ep.popDense, aes(fill=q5(totalPop)), color=NA)+
  scale_fill_manual(values = rev(palette5cont), 
                     labels = (qBr(acs_ep.popDense, "totalPop")),
                    name="Quintile\nBreaks") +
...
brio <- classified[classified$type %in% "BRIO", ]
...
brioMap <- ggplot()+
  geom_sf(data=el_paso, fill='lightgrey', color="black", size=20)+
  geom_sf(data=brio, color=c(palette6[6]), size=2)+
  labs(title="BRIO")+
  mapTheme
)
```
...
# Project the data to a suitable projection for distance calculations
el_paso <- el_paso %>%
...
stops_sf <- stops_sf %>%
  mutate(buffer = st_buffer(., dist = 0.5))

# Compute the union of all buffers
buffers_union_sf <- st_union(stops_sf$buffer.geometry)

# Compute the intersection of the buffer union and El Paso
within_buffers_sf <- st_intersection(buffers_union_sf, el_paso_sf)

# Compute the difference between the buffer union and El Paso
outside_buffers_sf <- st_difference(buffers_union_sf, el_paso_sf)
```
...
```{r}
localCentroid <- ridership_net_local %>%
