...
lm_best_params     <- select_best(lm_tuned, metric = "mae"    )
glmnet_best_params <- select_best(glmnet_tuned, metric = "mae")
rf_best_params     <- select_best(rf_tuned, metric = "rmse"    )
xgb_best_params    <- select_best(xgb_tuned, metric = "mae"   )
...
output$pred <- predict(full_fit_rf, new_data = input) %>% exp()
...
output_sf <- left_join(output, hex) %>% st_sf()
