...
grid.arrange(ncol=2,
             ggplot()+
               geom_sf(data=acs_ep1, aes(fill=whitePopPct), color=NA)+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
               labs(title="% White population",
                    caption="Data: ACS, 2020")+
               mapTheme,
             ggplot()+
               geom_sf(data=acs_ep1, aes(fill=hlPopPct), color=NA)+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
               labs(title="% Hispanic/Latino population",
                    caption="Data: ACS, 2020")+
               mapTheme,
             ggplot()+
               geom_sf(data=acs_ep1, aes(fill=blackPopPct), color=NA)+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
               labs(title="% Black population",
                    caption="Data: ACS, 2020")+
               mapTheme,
             ggplot()+
               geom_sf(data=acs_ep1, aes(fill=asianPopPct), color=NA)+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
...
# <- left_join(acs_ep, tx_work, by=c('GEOID' = 'w_tract'))
ep_work_census <- acs_ep
#ep_work_census <- ep_work_census %>%
 # select(-starts_with("CF"))

ep_work_census_st <- ep_work_census %>% 
  dplyr::select(-c("GEOID", "NAME", "OBJECTID", "NAME.1", "GlobalID", "Shape_Length", "Shape_Area","B25044_001E"
                 ))
...
census_lehd_hex <- st_interpolate_aw(ep_work_census_st, hex, extensive = T) %>%
  mutate(uniqueID = rownames(.))
...
final_hex <- final_hex %>% filter(!uniqueID %in% c(188, 365))
...
rf_grid <- expand.grid(mtry = seq(10,100,20), 
                       min_n = c(10,30,60))
xgb_grid <- expand.grid(mtry = seq(10,100,20), 
                       min_n = c(10,30,60))
...
models_mae <- data.frame(model = c("Random Forest", "XG Boost"),
...
```{r export hex with prediction}
output_xgb_sf <- output_xgb_sf 
#st_write(output_xgb_sf, 'final_hex2.geojson', overwrite = T, append = F)
```
