...
acs_ep_hex <- acs_ep %>% dplyr::select(-GEOID) %>% st_interpolate_aw(hex, extensive = T) %>%
  mutate(uniqueID = rownames(.))
...
#st_write(output_xgb_sf, 'final_hex3.geojson', overwrite = T, append = F)
```
...
st_write(final_data, 'final_hex4.geojson', overwrite = T, append = F)
```
