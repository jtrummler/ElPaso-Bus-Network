...
ridership_net_local <-  %>% # Local Bus RT ridership
...
busbaysnet <- hex %>% st_join(local_transit_bays %>% mutate(stop1 = 1)) %>% mutate(stop1 = replace_na(stop1, 0)) %>% group_by(uniqueID) %>% summarise(stops_in_hex = sum(stop1))
...
## metrics across grid
# autoplot(pzinf_tuned)
# collect_metrics(x_tuned)
```
...
# #1 predicting while staying in tidymodels
predict(full_fit_rf, new_data = ames[3,]) %>% 
  mutate(.pred_original = exp(.pred))

# #2 extract final model object
...
# predicting on extracted model object
MY_DATA_juiced <- prep(MY_DATA) %>% bake() %>% juice()
predict(rf_full_mod, new_data = MY_DATA_juiced)
```
