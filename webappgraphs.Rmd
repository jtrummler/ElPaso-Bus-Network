...
final_hex <- read_sf("final_hex5.geojson")
...
data_all <- final_hex %>% mutate( highlight = ifelse(uniqueID == sample(1:615,1), "y","n"))
...
merge <- st_join(riderstops, data_all, join = st_intersects) %>% st_drop_geometry()
