...
Our data is aggregated to hexbins, which are equally shaped hexbins that act as an overlay on top of the city of El Paso boundary. Total ridership is aggregated into each hexbin to predict what the total ridership is based on the features engineered into our model. We built two models: a Random Forest model and an XGBoost model.

We are building this model to inform our client's interest in knowing how bus transit alternative routes will affect route-level and system-level equity and fare revenue implications. In our model, we bake in equity metrics, which highlight the ability for all residents, and especially transit-dependent populations, to have operable, efficient service, in addition to the accessibility of stops for all El Paso residents. Fare revenue optimization is derived from ridership. A Sun MEtro planner could look at both current routes to see if our predicted ridership is higher or lower than current ridership to understand if there is increased demand for bus routes, or submit new routes to understand route level predicted ridership and how it compares to existing routes.If our model can accurately predict bus transit ridership, it can inform how new transit routes perform on these indicators, which the web application we develop will evaluate.
...
**Informational network web app:** A web app where a Sun Metro planner can input their own .kml file of a new transit line and assess it on fare revenue (ridership) and equity (different independent variables assessing demographic distributions) optimization.
...
```{r current network, fig.width=12, warning=FALSE, message=FALSE, results=FALSE}
...
Next, we looked at current route ridership. We aggregated the dataset to the route level and calculated the total ridership. 
...
Local routes make up the majority of transit roadway infrastructure, and they are the only routes that are relatively unfixed. They also have the most political malleability - transit planning is a very political act, and can often have a lot of political bias in the route methodology, but through our data-oriented approach, we can successfully build an unbiased informational network so transit planners can leverage our findings to build a more interconnected, equitable route network.

While there are a handful of local ridership routes that have comparable ridership to other high ridership routes, the trend teeters out, and most local routes experience relatively low ridership. This can be attributed to a variety of factors: there are low demand in these areas, infrequent service, inaccessibility, or other.
...
Spatial lags in the context of bus ridership at stops along a route refer to the influence that the level of bus ridership at one stop has on the level of ridership at neighboring stops. In other words, the spatial lag effect suggests that the level of ridership at a particular stop is not only influenced by the characteristics of that stop, but also by the level of ridership at nearby stops. This can play a significant role in understanding ridership patterns in space. We use the three most populous local routes - 14, 50, and 65 - to understand this phenomena.

```{r spatial lags, warning=FALSE, message=FALSE, results=TRUE, fig.width=12}
route14 <- RT_stop_avg %>%
  filter(RT == 14)
route50 <- RT_stop_avg[RT_stop_avg['RT'] == 50]
route65 <- RT_stop_avg[RT_stop_avg['RT'] == 65]

ggplot()+
  geom_sf(data=route14, aes(size=avg_ons, fill=avg_ons, color=avg_ons))+
  scale_size_continuous(breaks=c(0, 15, 30, 45, 60, 75))+
  scale_fill_continuous(breaks=c(0, 15, 30, 45, 60, 75))


library(scales)
ggplot() +
  geom_sf(data=route14, aes(fill=avg_ons, color=avg_ons, size=avg_ons)) +
  scale_fill_continuous(name="Average Ons", breaks=c(0, 15, 30, 45, 60), fill=c(palette5cont)) +
  scale_color_continuous(name="Average Ons", breaks=c(0, 15, 30, 45, 60, 75), fill=c(palette5cont)) +
  scale_size_continuous(name="Average Ons", breaks=c(0, 15, 30, 45, 60, 75)) +
  guides(fill = guide_colorbar(title.position = "top", title.hjust = 0.5)) +
  labs(title = "Route 14 Average Ons by Stop")


ggplot() +
  geom_sf(data=route14, aes(size=avg_ons, fill=avg_ons, color=avg_ons)) +
  scale_size_continuous(name="Total Ridership", breaks=c(0, 15, 30, 45, 60, 75), guide = guide_legend()) +
  scale_fill_continuous(name="Average Ons", breaks=c(0, 15, 30, 45, 60, 75)) +
  scale_color_continuous(name="Average Ons", guide = guide_none()) +
  guides(fill = guide_legend(title = "Average Ons", title.position = "top", 
                             label.position = "bottom", barwidth = 15),
         color = guide_legend(title = "Average Ons", title.position = "top", 
                              label.position = "bottom", barwidth = 15)) +
  labs(title = "Route 14 Average Ons by Stop")
...
####
INSERTION OF LEAFLET CODE AND/OR LINK TO LEAFLET
####

# Feature Engineering

>DISCUSSION OF FEATURES CHOSEN<

>TABLE OF FEATURES<

## Spatial Unit: Hexbins

>DISCUSSION OF HEXBINS, THE PROCESS OF CHOOSING ETC.<
...
ggplot()+
  ...
  geom_sf(data=final_hex, fill=c(palette1), alpha=0.7)+
...
```
```{r}
jenks <- classIntervals(final_hex$ridership, n = 5, style = "jenks")

# Create categorical variable based on jenks breaks
final_hex$quintile <- cut(final_hex$ridership, jenks$brks, labels = FALSE)

# Plot data with jenks breaks
ggplot()+
  geom_sf(data=el_paso, fill='grey')+
  geom_sf(data=final_hex, aes(fill=as.factor(quintile)), alpha=0.7, color=NA)+
  scale_fill_manual(values = palette5cont, 
                    labels = jenks$brks,
                    name="Ridership\nQuintiles")+
  labs(title="Hex Bin Grid",
...
ggplot()+
  geom_sf(data=el_paso, fill='grey')+
  geom_sf(data=final_hex, aes(fill=q5(ridership)), alpha=0.7, color=NA)+
  scale_fill_manual(values = palette5cont,
                    labels=qBr(ridership),
                    name="Ridership\nQuintiles")+
  labs(title="Hex Bin Grid",
       subtitle="El Paso, Texas",
       caption="Total ridership for the year 2022 will be aggregated into each hexbin as the dependent variable,\nin addition to our predictive features.")+
...
ggplot()+ 
  ...
  geom_sf(data = final_hex, aes(fill = q5(ridership)), color=NA)+
  scale_fill_manual(values = palette5cont, 
                     labels = qBr(final_hex, "ridership"),
                    name="Ridership\nQuintiles") +
  labs(title="Total Local Ridership by Census Tract",
       subtitle="El Paso, TX (2022)")+
  mapTheme

# Cluster the ridership data into 5 groups
final_hex$ridership_clusters <- kmeans(final_hex$ridership, centers = 5)$cluster

# Create the ggplot
ggplot() +
  geom_sf(data = el_paso, fill = "lightgrey", color = "black", size = 0.2) +
  geom_sf(data = final_hex, aes(fill = ridership_clusters), color = NA) +
  scale_fill_discrete(values = palette5cont,
                     name = "Ridership\nClusters",
...
grid.arrange(ncol=2,
             ggplot()+
               geom_sf(data=acs_ep_plots, aes(fill=whitePopPct), color="black")+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
               labs(title="% White population",
                    caption="Data: ACS, 2019")+
               mapTheme,
             ggplot()+
               geom_sf(data=acs_ep_plots, aes(fill=hlPopPct), color="black")+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
               labs(title="% Hispanic/Latino population",
                    caption="Data: ACS, 2019")+
               mapTheme,
             ggplot()+
               geom_sf(data=acs_ep_plots, aes(fill=blackPopPct), color="black")+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
               labs(title="% Black population",
                    caption="Data: ACS, 2019")+
               mapTheme,
             ggplot()+
               geom_sf(data=acs_ep_plots, aes(fill=asianPopPct), color="black")+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
               labs(title="% Asian population",
                    caption="Data: ACS, 2019")+
               mapTheme,
             top="Racial Distribution in El Paso, TX"
)
```

```{r race distribution hex, warning=FALSE, message=FALSE, results=FALSE, fig.width=12}
grid.arrange(ncol=3,
             ggplot()+
               geom_sf(data=final_hex4, aes(fill=totalPop), color=NA)+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="per \nsq km")+
               labs(title="Total Population")+
               mapTheme3,
             ggplot()+
               geom_sf(data=final_hex4, aes(fill=whitePop), color=NA)+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="per \nsq km")+
               labs(title="White Population")+
               mapTheme3,
             ggplot()+
               geom_sf(data=final_hex4, aes(fill=hlPop), color=NA)+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="per \nsq km")+
               labs(title="Hispanic/Latino Population")+
               mapTheme3,
             ggplot()+
               geom_sf(data=final_hex4, aes(fill=blackPop), color=NA)+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="per \nsq km")+
               labs(title="Black Population")+
               mapTheme3,
             ggplot()+
               geom_sf(data=final_hex4, aes(fill=asianPop), color=NA)+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="per \nsq km")+
               labs(title="Asian Population")+
               mapTheme3,
             ggplot()+
               geom_sf(data=final_hex4, aes(fill=(nhPop+aiPop+otherRacePop+twoPlusRacePop)), color=NA)+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="per \nsq km")+
               labs(title="Other Race Population")+
               mapTheme3,
             top="Racial Distribution in El Paso, TX (Total Population per square mile)",
             bottom="Data: ACS, 2019"
)
```
...
ggplot()+
  geom_sf(data=acs_ep_plots, aes(fill=q5(vacantHUPct)), color="black")+
  scale_fill_manual(values = palette5cont, 
                    labels = (qBr(acs_ep_plots, "vacantHUPct")),
                    name="% Vacancy\nQuintiles") +
...
ggplot()+
  geom_sf(data=acs_ep, aes(fill=q5(medHHInc)), color="black")+
  scale_fill_manual(values = palette5cont, 
                    labels = (qBr(acs_ep_plots, "medHHInc")),
                    name="Income by\nQuintiles") +
...
```{r}
ggplot()+
               geom_sf(data=hex6cleaned, aes(fill=(disability)), color=NA)+
               scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="Total")+
               labs(title="Disabled Population", caption="Data: ACS, 2019")+
               mapTheme3
```
...
grid.arrange(ncol=2,
ggplot()+
  geom_sf(data=final_hex4, aes(fill=(jobsPopBalance)), color=NA)+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="Score",
                         breaks=c(0, 0.25, 0.5, 0.75, 1))+
  labs(title="Jobs to Population Balance")+
  mapTheme3,
ggplot()+
  geom_sf(data=final_hex4, aes(fill=(employmentHHMix)), color=NA)+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="Score",
                         breaks=c(0, 0.25, 0.5, 0.75, 1))+
  labs(title="Employment Household Mix")+
  mapTheme3,
bottom="Data: NWI, 2019"
)
```
...
grid.arrange(ncol=3,
ggplot()+
  geom_sf(data=final_hex4, aes(fill=(major_road.nn1)), color=NA)+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="Score", breaks=c(0, 1))+
  labs(title="Distance to Nearest Major Road")+
  mapTheme3,
ggplot()+
  geom_sf(data=final_hex4, aes(fill=(parks.nn1)), color=NA)+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="Score", breaks=c(0, 1))+
  labs(title="Distance to Nearest Park")+
  mapTheme3,
ggplot()+
  geom_sf(data=final_hex4, aes(fill=(department_stores.nn1)), color=NA)+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="Score", breaks=c(0, 1))+
  labs(title="Distance to Nearest Department Store")+
  mapTheme3,
top="Distance to Amenity Features (first nearest neighbor)", bottom="Data: Open Data El Paso, Open Street Map"
)
```
...
grid.arrange(ncol=2,
ggplot()+
  geom_sf(data=final_hex4, aes(fill=(stops_in_hex)), color=NA)+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="Total")+
  labs(title="Number of Bus Stops")+
  mapTheme3+theme(legend.position = "bottom"),
ggplot()+
  geom_sf(data=final_hex4, aes(fill=(bays_in_hex)), color=NA)+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="Total", )+
  labs(title="Number of Transfer Bay")+
  mapTheme3+theme(legend.position = "bottom")
)

grid.arrange(ncol=2,
ggplot(final_hex4, aes(x = stops_in_hex, y = ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Bus Stops per hex", y = "Total Ridership", 
       title = "Bus Stops per hex") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(ridership ~ stops_in_hex, data = final_hex4))$r.squared, 3)))+
  plotTheme2,
ggplot(final_hex4, aes(x = bays_in_hex, y = ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Transfer Bay Stops per hex", y = "", 
       title = "Transfer Bay Stops per hex") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(ridership ~ bays_in_hex, data = final_hex4))$r.squared, 3)))+plotTheme2
)
```
...
```{r acs scatters, fig.height=12, fig.width=18, warning=FALSE, message=FALSE, results=FALSE}
acs_scatters <- acs_correlations %>% 
  dplyr::select(c("total_ridership", "totalPop", "hlPop", "whitePop", "blackPop", "hlPop", "aiPop", "asianPop", "nhPop", "otherRacePop", "twoPlusRacePop", "medHHInc", "totalHU", "occupiedHU", "grossRentPerInc", "vehiclesAvailable", "disability", "lessThanHS", "highSchool", "associatesDeg", "bachelorDeg", "professionalDeg"))

ind_vars_acs <- c("totalPop", "hlPop", "whitePop", "blackPop", "hlPop", "aiPop", "asianPop", "nhPop", "otherRacePop", "twoPlusRacePop", "medHHInc", "totalHU", "occupiedHU", "grossRentPerInc", "vehiclesAvailable", "disability", "lessThanHS", "highSchool", "associatesDeg", "bachelorDeg", "professionalDeg")

grid.arrange(ncol=5, top="ACS Variables vs. Total Ridership",
ggplot(acs_scatters, aes(x = totalPop, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Total Population", y = "Total Ridership", 
       title = "Total Population") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ totalPop, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = whitePop, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "White Population", y = "Total Ridership", 
       title = "White Population") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ whitePop, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = hlPop, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Hispanic/Latino Population", y = "Total Ridership", 
       title = "Hispanic/Latino Population") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ hlPop, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = blackPop, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Black Population", y = "Total Ridership", 
       title = "Black Population") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ blackPop, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = asianPop, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Asian Population", y = "Total Ridership", 
       title = "Asian Population") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ asianPop, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = medHHInc, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Median Household Income", y = "Total Ridership", 
       title = "Median Household Income") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ medHHInc, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = occupiedHU, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Occupied Housing Units", y = "Total Ridership", 
       title = "Occupied Housing Units") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ occupiedHU, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = grossRentPerInc, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Gross Rent per Income", y = "Total Ridership", 
       title = "Gross Rent per Income") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ grossRentPerInc, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = vehiclesAvailable, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Vehicles per sq km", y = "Total Ridership", 
       title = "Vehicles per sq km") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ vehiclesAvailable, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = disability, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Disabled Population", y = "Total Ridership", 
       title = "Disabled Population") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ disability, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = lessThanHS, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Less than HS", y = "Total Ridership", 
       title = "Less than HS") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ lessThanHS, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = highSchool, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "High School", y = "Total Ridership", 
       title = "High School") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ highSchool, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = associatesDeg, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Associates Degree", y = "Total Ridership", 
       title = "Associates Degree") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ associatesDeg, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = bachelorDeg, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Bachelor Degree", y = "Total Ridership", 
       title = "Bachelor Degree") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ bachelorDeg, data = acs_scatters))$r.squared, 3)))+
  plotTheme2,
ggplot(acs_scatters, aes(x = professionalDeg, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Prof. Degree", y = "Total Ridership", 
       title = "Prof. Degree") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ professionalDeg, data = acs_scatters))$r.squared, 3)))+
  plotTheme2
)
```

####
CODE TO JOIN ACS TO HEXES
####
...
grid.arrange(ncol=2,
ggplot(nwi_bg_st, aes(x = nearestTransit)) +
...
ggplot()+
  geom_sf(data=el_paso, fill="lightgrey", color="black")+
  geom_sf(data=nwi_bg, aes(fill=q5(nearestTransit)), color="black")+
  scale_fill_manual(values = palette5cont, 
                    labels = (qBr(nwi_bg, "nearestTransit")),
...
ggplot()+
  geom_sf(data=el_paso, fill="lightgrey", color="black")+
  geom_sf(data=nwi_bg, aes(fill=q5(destinationAccessibility)), color="black")+
  #scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="Density\nScore")+
  scale_fill_manual(values = palette5cont, 
                    labels = (qBr(nwi_bg, "destinationAccessibility")),
                    name="Density Score") +
...
ggplot(nwi_bg_st, aes(x = intersectionDensity)) +
  ...
  plotTheme,
ggplot()+
  geom_sf(data=nwi_bg, aes(fill=q5(intersectionDensity)), color="black")+
  #scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="Density\nScore")+
  scale_fill_manual(values = palette5cont, 
                    labels = (qBr(nwi_bg, "intersectionDensity")),
                    name="Density Score"
                    ) +
...
```{r nwi scatters, fig.width=12, fig.height=8, warning=FALSE, message=FALSE, results=FALSE}
local1 <- df[df$type %in% "Local", ]
...
ep_blocks <- ep_blocks %>% # EP TRACTS ARE NOW CRS 4269
  st_transform(crs=4269)

local_agg_blocks <- st_join(local_agg, ep_blocks, join = st_within)

local_agg_blocks <- local_agg_blocks %>%
  group_by(GEOID) %>%
  summarise(total_ridership = sum(total_ridership))
...
agg_data3 <- left_join(local_agg_blocks, ep_tracts, by = "GEOID") %>%
  dplyr::select(c("total_ridership", "GEOID"))

nwi_correlations <- merge(agg_data3, nwi_bg_st, on = "GEOID")
nwi_correlations_log <- nwi_correlations[nwi_correlations$nearestTransit != -1,]
nwi_correlations_log[nwi_correlations_log == 0] <- as.numeric(nwi_correlations_log[nwi_correlations_log == 0 + 0.1])
 



grid.arrange(ncol=4, top="NWI Variables vs. Total Ridership",
ggplot(nwi_correlations, aes(x = pct0Car, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "% 0 Car Households", y = "Total Ridership", 
       title = "") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ pct0Car, data = nwi_correlations))$r.squared, 3)))+
  plotTheme2,

ggplot(nwi_correlations, aes(x = pct1Car, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "% 1 Car Households", y = "Total Ridership", 
       title = "") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ pct1Car, data = nwi_correlations))$r.squared, 3)))+
  plotTheme2,

ggplot(nwi_correlations, aes(x = pctLowWage, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "% Low Wage Workers", y = "Total Ridership", 
       title = "") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ pctLowWage, data = nwi_correlations))$r.squared, 3)))+
  plotTheme2,

ggplot(nwi_correlations, aes(x = employmentDensity, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Employment Density", y = "Total Ridership", 
       title = "") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ employmentDensity, data = nwi_correlations))$r.squared, 3)))+
  plotTheme2,

ggplot(nwi_correlations, aes(x = employmentHHMix, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Employment Household Mix", y = "Total Ridership", 
       title = "") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ employmentHHMix, data = nwi_correlations))$r.squared, 3)))+
  plotTheme2,

ggplot(nwi_correlations, aes(x = nearestTransit, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Nearest Transit Stop", y = "Total Ridership", 
       title = "") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ nearestTransit, data = nwi_correlations))$r.squared, 3)))+
  plotTheme2,

ggplot(nwi_correlations, aes(x = jobAccessibility, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Job Accessibility", y = "Total Ridership", 
       title = "") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ jobAccessibility, data = nwi_correlations))$r.squared, 3)))+
  plotTheme2,

ggplot(nwi_correlations, aes(x = destinationAccessibility, y = total_ridership)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = palette1) +
  labs(x = "Destination Accessibility", y = "Total Ridership", 
       title = "") +
  annotate("text", x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
           label = paste0("R^2 = ", round(summary(lm(total_ridership ~ destinationAccessibility, data = nwi_correlations))$r.squared, 3)))+
  plotTheme2
)
...
grid.arrange(ncol=3,
ggplot()+
  geom_sf(data=lehd_ep, aes(fill=income1250LessPct*100), color="black")+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
  ...
  theme(legend.position = 'bottom'),
ggplot()+
  geom_sf(data=lehd_ep, aes(fill=income1251to3333Pct*100), color="black")+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
  ...
  theme(legend.position = 'bottom'),
ggplot()+
  geom_sf(data=lehd_ep, aes(fill=income3333OverPct*100), color="black")+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
...
```{r bar chart income dist,warning=FALSE, message=FALSE, results=FALSE, fig.height=8, fig.width=12}
epworkincome <- lhed_ep_st[, c("GEOID", "income1250LessPct", "income1251to3333Pct", "income3333OverPct")]
...
ggplot(epworktidy, aes(x = Income, y = GEOID, fill = IncomeLevel)) +
  ...
  scale_fill_manual(values=c(palette5cont[5:3]),
                    labels=c("< 1250", "Between 1251 and 3334", " > 3334"))+
  labs(y = "Census Tract", x = "Income", fill = "Monthly Income ($)", title = "Job Income Breakdown at Census Tract Level",
       subtitle = "El Paso, TX",
       caption = "Data: LEHD, 2019") +
  plotTheme+
...
ggplot()+
  geom_sf(data=lehd_ep, aes(fill=(age29youngerPct*100)), color="black")+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
  labs(title="Age < 29",
       caption="Data: LEHD, 2019")+
  mapTheme+
  theme(legend.position = 'bottom'),
ggplot()+
  geom_sf(data=lehd_ep, aes(fill=(age30to54Pct*100)), color="black")+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
  ...
  theme(legend.position = 'bottom'),
ggplot()+
  geom_sf(data=lehd_ep, aes(fill=(age55olderPct*100)), color="black")+
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1, name="%")+
  labs(title="Age 55+",
       caption="Data: LEHD, 2019")+
  mapTheme+
...
```{r bar chart age dist,warning=FALSE, message=FALSE, results=FALSE, fig.height=8, fig.width=12}
epworkage <- lhed_ep_st[, c("GEOID", "age29youngerPct", "age30to54Pct", "age55olderPct")]

epworktidy2 <- epworkage %>%
  gather(key = "AgeBracket", value = "Age", -GEOID)

ggplot(epworktidy2, aes(x = Age, y = GEOID, fill = AgeBracket)) +
  geom_bar(stat = "identity", position = "stack", width=2) +
  scale_fill_manual(values=c(palette5cont[5:3]),
                    labels=c("< 1250", "Between 1251 and 3334", " > 3334"))+
  labs(y = "Census Tract", x = "Income", fill = "Monthly Income ($)", title = "Job Income Breakdown at Census Tract Level",
...
```{r parks data, warning=FALSE, message=FALSE, results=FALSE, fig.width=12}
parks <- parks %>%
...
```{r road classification and count roads, warning=FALSE, message=FALSE, results=FALSE, fig.width=12}
road_class <- c(
  "LOCAL" = "Local",
  "MINOR" = "Minor",
  "FREEWAY" = "Freeways",
...
```{r major roads map, warning=FALSE, message=FALSE, results=FALSE, fig.width=12}
major <- roads %>%
...
```{r different osm plots, warning=FALSE, message=FALSE, results=FALSE, fig.width=12}
...
## Spatial Effect - Transit Bays

>INFO<

```{r spatial effects, warning=FALSE, message=FALSE, results=FALSE, fig.width=12}
...
bay1 <- ggplot(bays_st, aes(x = Bay)) +
  ...
  labs(x = "Bay", y = "Count", title = "Number of Stops in Each Bay")+
...
bay2 <- ggplot()+
  ...
  labs(title="Transit Centers - El Paso, TX")+
  guides(color = guide_legend(override.aes = list(fill = palette8)))+
  mapTheme+ theme(legend.position = "bottom", legend.direction = "horizontal")
