...
>IN THIS SECTION, WE LOOK AT MORE ON THE CONTEXT OF OUR STUDY REGION AND THE DATA OUR CLIENT PROVIDED<
...
ggplot()+
  ...
  geom_sf(data=bus_routes,
          ...
          color="#C96A52FF", 
          size=1)+
  ggsn::scalebar(el_paso, location="bottomleft", dist_unit = 'mi', dist=2, height=0.01, transform=TRUE, model='WGS84')+
...
>DISCUSSION ON HOW OUR MODEL ONLY USES LOCAL ROUTES<

```{r context map with local routes, fig.width=12, warning=FALSE, message=FALSE, results=FALSE}
grid.arrange(ncol=2,
ggplot()+
  geom_sf(data=tx, fill='grey', color='black', size=0.5)+
  geom_sf(data=ep_co, fill=palette6[2])+
...
ggplot() +
  ...
  geom_sf(data=ft_bliss, fill=palette6[4], color=palette6[4]) +
  geom_sf(data=bus_routes, color="black") +
  geom_sf(data=local, color="red")+
  labs(caption="El Paso (county) in pink\nEl Paso (city) in grey\nRio Grande River in blue\nFranklin Mountains State Park in purple\nFort Bliss Military Base in olive\nCurrent bus network in black\nLocal routes in red")+
  mapTheme,
top="Context Map - El Paso, TX"
)
```
...
ggplot()+ 
  ...
  geom_sf(data = agg_data2, aes(fill = q5(total_ridership)), color="black")+
