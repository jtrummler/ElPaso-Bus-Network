...
census_vars <- elpaso %>% select(!starts_with('B') & ! c("GEOID","NAME","geometry")) %>% colnames()
walkscore_vars <- c()
...
```{r}
final_hex_cv <- input %>% mutate(
  cvID = sample(round(nrow(final_hex) / 24), size=nrow(final_hex), replace = TRUE)) 

cols <- input %>% select(ridership, lhodes_vars, census_vars, walkscore_vars) %>% colnames()

reg.cv <- crossValidate(
  dataset = final_hex_cv,
  id = "cvID",
  dependentVariable = "ridership",
  indVariables = cols) %>%
    dplyr::select(cvID = cvID, ridership, Prediction)
```
