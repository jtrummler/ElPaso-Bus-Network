...
sum_by_stops <- df %>%
  group_by(TP, longitude, latitude) %>%
...
```{r ridership per census tract}

ep_tx_tracts <- get_acs(
  geography = "tract", 
...
st_crs(ep_tx_tracts) <- st_crs(4269)
st_crs(sum_by_stops) <- st_crs(4269)
  

ridership_by_tract <- st_join(ep_tx_tracts, sum_by_stops, join = st_intersects)

riderhsip_by_tract <- ridership_by_tract %>%
  group_by(NAME) %>%
  summarize(ons = sum(total_ons), 
            offs = sum(total_offs)) %>%
  dplyr::f

# create two ggplot objects, one for ons and one for offs
p_ons <- ggplot() +
  geom_sf(data = riderhsip_by_tract, aes(fill = ons), color="transparent") +
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1) +
  labs(title = "Onboard Counts") +
  mapTheme

p_offs <- ggplot() +
  geom_sf(data = riderhsip_by_tract, aes(fill = offs), color="transparent") +
  scale_fill_paletteer_c("grDevices::Red-Yellow", -1) +
  labs(title = "Offboard Counts") +
  mapTheme

# arrange the two plots side by side using facet_wrap
gridExtra::grid.arrange(p_ons, p_offs, ncol = 2)
