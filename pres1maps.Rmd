...
briolines<-  read_sf(paste(data_folder, "/Brio.shp", sep = '')) %>% st_transform(crs = 4269)

elpaso <- read_sf(paste(data_folder, "/CityLimits.geojson", sep = '')) %>% st_transform(crs = 4269)
...
g <- ggplot()+
  geom_sf(data = briolines, size = 1.5, aes(color = Name))+
...
anim_save('citybusses1.gif', g, path = "C:/Users/cchue/Documents/Penn MUSA/Spring/Practicum",  duration = 15, fps = 10, bg = 'black')
